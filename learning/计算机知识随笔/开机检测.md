...
ESP(EFI系统分区): EFI System Partition，FAT格式，容量较小，主要目录是EFI，储存已安装系统的EFI引导加载程序以及启动时固件使用的应用程序
 EFI/boot/bootx64.efi是EFI默认的启动项。安装的操作系统会建立相应的目录EFI/xxx，并将自己的启动项复制为到EFI/boot/bootx64.efi作为缺省启动项。
