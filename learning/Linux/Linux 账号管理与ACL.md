...
|1~999(系统账号)|保留给系统使用的 ID。除了 0 之外，其他的 UID 权限与特性并没有差别。默认 1000 以下的数字让给系统作为保留账号只是一个习惯。由于系统上面启动的网络服务或背景服务希望使用较小的权限去运作，因此不希望使用 root 的身份去执行这些服务，所以我们就得要提供这些运作中程序的拥有者账号才行。这些系统账号通常是不可登入的。根据系统账号的由来，通常这类账号又约略被区分为两种：o 1~200：由 distributions 自行建立的系统账号；o 201~999：若用户有系统账号需求时，可以使用的账号 UID。|