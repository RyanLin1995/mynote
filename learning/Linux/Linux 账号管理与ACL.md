...
* **2. 密码：** 账号真正的密码，经过编码(加密) 。密码的长度不能修改，如果这个字段的长度被改变，该密码就会失效(算不出来)。因此很多软件透过这个功能，在此字段前加上 ! 或 * 改变密码字段长度，让密码暂时失效
...
* **7. 密码过期后的账号宽限时间(密码失效日)(与第 5 字段相比)：** 在密码过期几天后，如果使用者还是没有登入更改密码，那么这个账号的密码将会失效，即该账号再也无法使用该密码登入了。要注意密码过期与密码失效并不相同。
密码有效日期为 更新日期(第 3 字段) + 重新变更日期(第 5 字段)，过了该期限后用户依旧没有更新密码，那该密码就算过期了。虽然密码过期但是该账号还是可以用来进行其他工作的，包括登入系统取得bash。但当用户登入系统时，系统会强制要求必须要重新设定密码才能登入
