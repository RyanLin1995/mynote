...
用法：`awk '条件类型1{动作1} 条件类型2{动作2} ...' 文件名`

图解 awk:
![awk1.png](https://i.loli.net/2021/01/23/UNjpGKQJZC9fMtT.png)
...
### awk 命令演示
1. 如果想要知道 awk 命令目前正在处理第几行，可以在动作中使用 NR 变量({NR})
4. 如果想要知道 awk 命令目前正在处理的行数拥有多少个字段，可以在动作中使用 NF 变量({NF})
