...
* .*(点星)：代表匹配任何非换行字符
1. 匹配关键字为 g??d 的数据：`grep "g..d" 文件名`

2. 匹配字符中带有 "o" 、"oo"、"ooo"的数据：`grep "oo*" 文件名`、`grep "ooo*" 文件名`、`grep "oooo*" 文件名`(因为 * 指的是重复前一个字符，0到无穷多次的意思，所以匹配 "o" 的时候需要填写 "oo")

3. 匹配以 "g" 开头和以 "g" 结尾的字符串：`grep "g.*g" 文件名`

4. 匹配有数字的行：`grep "[1-9]." 文件名` 或 `grep "[1-9][1-9]*" 文件名` 或 `grep "[1-9]" 文件名`
