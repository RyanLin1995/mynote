...
一个例子:
通过建立大文件以制作 loop 装置: 制作出一个大文件，然后将这个文件格式化后挂载。即如果当初在分区时，你只有分区出一个根目录，假设你已经没有多余的容量可以进行额外的分区的，偏偏根目录的容量还很大。此时你就能够制作出一个大文件，然后将这个文件挂载。如此一来感觉上你就多了一个分区
1. 通过 `dd` 建立大文件: `dd if=/dev/ze`
