...
|选项与参数|说明|
|-|-|
|-n|不执行 script，仅查询语法问题，没有问题不输出任何信息|
|-v|执行 sccript 前，先将 scripts 的内容输出到屏幕上，即显示完整代码跟结果|
|-x|将使用到的 script 内容显示到屏幕上(类似于debug，即执行了哪句代码就显示哪句)|