...
|:set backup|是否自动储存备份档，如果设置了 backup, 那么当你改动任何文件时, 源文件会被另存为一个名为 filename~ 的文件, 默认值为 nobackup|
...
|:set|显示所有非默认值设定参数，即自行变更的参数|
|:syntax on/off|是否根据不同程序语言显示不同颜色|
|:set bg=dark|设置背景颜色为深色|
...
|-f|from，后面接原本的格式编码|
|-t|to，后面接需要转换到的编码|
|-o file|如果需要保留源文件，加上 -o 接文件名，则会生成新文件并保留源文件|