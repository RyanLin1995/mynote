...
### MYSQL 数据表格相关
1. 查看数据表: `show tables;`
2. 创建数据表: `create table 'tablename'('field(字段) type(类型) constraint(约束)','field(字段) type(类型) constraint(约束)')` 例如: `create table test(id int primary key not null auto_increment,name varchar(30));`
