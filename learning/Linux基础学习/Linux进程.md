...
以上是来自《鸟哥的Linux私房菜》关于 Linux PID 的介绍。也就是说，每个程序在被事件触发而进到内存中变为进程时，Linux 都会分配一个带有权限的号码给这个相关资源的集合(不仅仅是代码段(text section)，还包括文件，信号，CPU状态，内存地址空间等)。这个号码为 PID。因为不同用户的权限设置各有不同，所以不同用户调用同一个程序而生成的进程会有区别。
